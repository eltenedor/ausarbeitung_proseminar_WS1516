...
\begin{proof}
  ...
  Denn angenommen es gälte $|Y| \not\leq |\mathcal{P}(X)| $

  UNVOLLSTÄNDIG

  Es bezeiche $\aleph$ nach dem Satz von \textsc{Hartogs} zur Potenzmenge $\mathcal{P}(X)$ kleinste existierende Kardinalzahl mit $\aleph \not\leq \mathcal{P}(X)$
  Man beachte, dass die Existenz von $\aleph$ bereits in \ZF gilt.
  Dann gilt nach dem zuvor Bewiesenen auch $\aleph \not\leq^* |X|$ was zusammen mit (2) $|X|\leq^* \aleph$ impliziert.
  ...
  \end{displaymath}
  gilt, setzt und für eine nichtleere Teilmenge von $T \subseteq X$ das aufgrund der Injektivität von $g$ eindeutig bestimmte $x \in g(T)$ wählt mit $g(x) = \min f^{-1}(\{x\}) = \min g(T)$.
...
\begin{proof}
  ...
  Als solche sind sind sie aber auch kompakt.
  ...
  Es gilt 
  \begin{displaymath}
    \prod_{i \in I} \mathbf{2}^{C(X_i,\mathbf{2})} \simeq \mathbf{2}^{C(X_i,\mathbf{2}) \times \{i\}}.
  \end{displaymath}
