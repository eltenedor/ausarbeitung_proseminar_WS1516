...
\begin{proof}
  ...
  Damit lässt sich nun Lemma \ref{lem:filterclusterpoint}(i) auf $\mathcal{F}$ anwenden und es folgt für die Menge $\mathfrak{A}$ aller offen-abgeschlossenen Elemente von $\mathcal{F}$
  \begin{displaymath}
  ...
  Die Abbildung $f$ ist injektiv, denn für $x,y \in P$ mit $x \neq y$ gilt mit $A_{(x,y)} \in \mathfrak{A}$ aus Lemma \ref{lem:filterclusterpoint}(ii) 
  \begin{displaymath}
    f_{A_{(x,y)}}(x) = 0 \neq 1 = f_{A_{(x,y)}}(y).
  \end{displaymath}

  Zudem ist $f$ offen bezüglich der Spurtopologie auf $F$.
  Um Lemma \ref{lem:openmap} anwenden zu können sei nun $O = \pi_{i_0}^{-1}[\{j\}]$ mit $i_0 \in I$ und $j \in \{0,1\}$ eine beliebige offene Subbasismenge der Produkttopologie auf $P$.
  Es ist $O$ eine offen abgeschlossene Menge. 
  Fall des Weiteren $O \in \mathcal{F}$, also insbesondere $O \in \mathfrak{A}$ gilt, so ist für alle $x \in O$
  \begin{displaymath}
    \pi_O(f(x)) = f_O(x) = 1,
  \end{displaymath}
  Daraus folgt 
  ...
  Sei im anderen Fall $O \not\in \mathcal{F}$, so gilt entweder, dass für alle $F \in \mathcal{F}$
  \begin{displaymath}
    F \cap O \neq \emptyset
  \end{displaymath}
  ...
  Es ist daher $\mathbf{2}^I$ als Produkt ultrafilterkompakter Räume wiederum ultrafilterkompakt.
  ...
  Sei $U_p \subseteq \mathbf{2}^\mathfrak{A}$ eine beliebige Umgebung von $p$.
  Dann existieren Subbasiselemente $B_{A_1},\dots,B_{A_n}$ mit
  \begin{displaymath}
  ...
  \end{displaymath}
  gilt $f_{A_i}(x) = 1$, da offen-abgeschlossene Mengen sowie Filter eine endliche Durchschnittseigenschaft besitzen, ist
  \begin{displaymath}
  \end{displaymath}
